...
graph TD
    ...
    classDef api fill:#ff9f43,stroke:#333,stroke-width:1px,color:#fff;
    classDef ws fill:#1dd1a1,stroke:#333,stroke-width:1px,color:#fff;
    classDef ui fill:#5f27cd,stroke:#333,stroke-width:1px,color:#fff;
    ...
    subgraph Bridge ["LISTEN/NOTIFY Bridge"]
        API[("DCM API<br/>Bun + Hono<br/>Port 3847")]:::api
        WS[("DCM WebSocket<br/>Bun + Native<br/>Port 3849")]:::ws
        Dash[("DCM Dashboard<br/>Next.js 16<br/>Port 3848")]:::ui
    end
...
sequenceDiagram
    ...
    participant Hooks as Hooks (Bash)
    participant API as DCM API
    participant DB as PostgreSQL

    Note over Claude, DB: Session Initialization
    Claude->>Hooks: SessionStart(startup)
    activate Hooks
    Hooks->>Hooks: ensure-services.sh
    Hooks->>API: GET /health (Check)
    opt Services Not Running
        Hooks->>API: Start Background Process
    end
    Hooks->>API: POST /sessions (track-session)
    deactivate Hooks

    Note over Claude, DB: Active Workflow
    loop Tool Usage
        Claude->>Hooks: PostToolUse(*)
        Hooks->>API: POST /api/actions
        
        opt Every 10th Call
            Claude->>Hooks: PostToolUse(monitor)
            Hooks->>API: Check Transcript Size
        end
    end

    Note over Claude, DB: Context Compaction
    Claude->>Hooks: PreCompact
    activate Hooks
    Hooks->>API: POST /compact/save
    API->>DB: Snapshot Active Tasks & Files
    deactivate Hooks

    rect rgb(30, 30, 30)
        Note right of Claude: COMPACTION HAPPENS
    end

    Claude->>Hooks: SessionStart(compact)
    activate Hooks
    Hooks->>API: GET /context/brief
    API-->>Hooks: Return Context JSON
    Hooks-->>Claude: Inject Context (additionalContext)
    deactivate Hooks

    Note over Claude, DB: Completion
 ...
 * Before compact: pre-compact-save.sh saves active tasks, modified files, agent states, and key decisions to the database
 * After compact: post-compact-restore.sh fetches a context brief and injects it back into the session via additionalContext
 * Proactive monitoring: monitor-context.sh runs every 10th tool call; if the transcript exceeds 800 KB, it triggers an early snapshot so data is saved even if compaction happens unexpectedly
Cross-agent sharing
 ...
 * Health check -- Calls GET /health on the API. If healthy, exits immediately (no-op).
 * Lock acquisition -- Creates a lock file to prevent concurrent auto-starts when multiple Claude sessions launch simultaneously.
 * PostgreSQL check -- Verifies PostgreSQL is reachable via pg_isready. If not, logs a warning and exits gracefully.
 * Service start -- Launches the API and WebSocket servers as background processes.
 * Readiness wait -- Polls the health endpoint for up to 5 seconds until the API confirms healthy status.
