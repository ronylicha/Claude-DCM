...
stdbuf -oL -eL claude -p "$USER_MSG" --system-prompt-file "${promptFile}" --model "${model}" --output-format stream-json --max-turns 15 --tools "Read,Bash,Grep,Glob" < /dev/null > "${outputFile}" 2> "${errorFile}"
echo $? > "${doneFile}"
`;
      case "codex":
        return `#!/bin/bash
