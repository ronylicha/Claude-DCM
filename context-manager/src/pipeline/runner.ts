...
export async function createPipeline(
  ...
  // Insert pipeline row (UUID auto-generated by PostgreSQL)
  const [pipeline] = await sql<PipelineRow[]>`
    INSERT INTO pipelines (session_id, name, status, input, plan, current_wave, config)
    VALUES (
      ${sessionId},
      ${plan.name},
      'ready',
  ...
  log.info(
    `Created pipeline: id=${pipelineId}, session=${sessionId}, ` +
    `waves=${plan.waves.length}, steps=${totalSteps}`,
  );

  // Insert all steps from the plan (UUIDs auto-generated)
